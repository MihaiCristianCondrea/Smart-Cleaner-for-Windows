...
public sealed class RegistryDiskCleanupOperations : IDiskCleanupAnalyzer, IDiskCleanupExecutor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (status < 0)
            {
                return HandlerExecutionResult.Failed(CreateErrorMessage(status));
            }

            var callback = new DiskCleanupCallback(cancellationToken);
            var result = cache.Purge(0, callback);

            if (result == HResults.E_ABORT && cancellationToken.IsCancellationRequested)
            {
                throw new OperationCanceledException(cancellationToken);
            }
            ...
            if (result < 0)
            {
                return HandlerExecutionResult.Failed(CreateErrorMessage(result));
            }
