...
Add the MSIX visual assets to this folder before building the packaging project. Use PNG files with
the following names and base (100% scale) dimensions:
...
Provide higher scale assets (e.g., `*.scale-200.png`) if desired. The packaging project references
these files directly, so they must exist for the MSIX build to succeed.
